...
window.onscroll = function () {
  ...
  let articles = document.getElementsByClassName("relToViewPort");
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    if (domRect.top < window.innerHeight) {
      ...
      articles[i].setAttribute("class", "specialBlinderAnimation specialBlinder1 relToViewPort");

      // falls der untere Punkt im Minus liegt ist es oben rausgerutscht
      if (domRect.bottom < 0) {
        console.log("ich bin oben rausgerutscht");
        articles[i].classList.remove("specialBlinderAnimation");
